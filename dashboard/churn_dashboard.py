...
def calculate_metrics(df):
    ...
    try:
        ...
        # Revenue at risk
        revenue_at_risk = df[df['churn_risk_level'] == 'high']['monthly_revenue'].sum() * 12 if 'churn_risk_level' in df.columns else 0
...
def main():
    ...
    with col2:
        ...
        delta_color = "inverse" if churn_rate > 15 else "normal"
        st.metric(
            "Churn Rate", 
            f"{churn_rate:.1f}%",
    ...
    with col4:
        st.metric(
            ...
            delta=f"{np.random.randint(-10, 5):+d}",
            delta_color="inverse",
    ...
    with tab3:
        ...
        if 'features_adopted' in df.columns:
